...
void main()
{
    ...
    uint second = first + 1;
    imageStore(luma_out, ivec2(x, row),
               uvec4(quantize_code(64.0 + 876.0 * pixel_luma(first),
                                   64.0, 940.0, 0, first)));
    imageStore(luma_out, ivec2(x + 1, row),
               uvec4(quantize_code(64.0 + 876.0 * pixel_luma(second),
                                   64.0, 940.0, 0, second)));
    uint chroma = row * (params.width / 2) + pair;
    imageStore(cb_out, ivec2(pair, row),
               uvec4(quantize_code(512.0 + 896.0 * filtered_cb(x, row),
                                   64.0, 960.0, 1, chroma)));
    imageStore(cr_out, ivec2(pair, row),
               uvec4(quantize_code(512.0 + 896.0 * filtered_cr(x, row),
                                   64.0, 960.0, 2, chroma)));
}
