...
- [ ] Batch D.1-A：从正常 E2E 移除完整 MOV packet scan，分离 startup/conversion/validation/
      process timers，并消除重复 full-resolution Vulkan preflight；
- [ ] Batch D.1-B：修正并报告 FFmpeg requested/effective async depth，使用真实帧单进程
      depth 1/2/4/8 做 matched A/B；未证明收益前不实施 queue separation；
- [ ] Batch D.1-C/D：只在前置 telemetry 证明可回收 idle 或 PCIe/heap 问题后，分别实验
      queue reservation 与 device-local RAW staging；未超出噪声则记录 no-go；
- [ ] Batch E：按 E-A～E-F 更新 compression 6/7 official truth、修复 RAW buffer 过度分配、
      实现持久 decoder worker pool、条件实施 CPU 快速 decoder，并在完整流水中验证
      90/120/130 fps 容量分档；GPU pipeline 边界冻结在 U16 RAW upload；
- [ ] 性能开发期间持续推进 NLE/硬件/长时间发布 gates。

Stage 0～3 / Batch A～D 已完成。下一项 GPU pipeline 工程任务是 **Batch D.1-A：拆分
output validation 与 conversion timing，并删除重复 full-resolution Vulkan preflight**；随后
按 D.1-B 的真实帧 depth matrix 决定 async/queue 工作是否继续。CPU decoder 主线仍从
**Batch E-A：更新并冻结同时覆盖 compression 6/7 的 official truth source** 开始；在 E-A 和
更新后 baseline 完成前，不开始自研 SIMD decoder。
