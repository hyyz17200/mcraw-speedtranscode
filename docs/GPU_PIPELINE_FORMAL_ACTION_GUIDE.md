...
- [ ] Batch E-F compression 6 corpus gate：仍需真实 compression 6 素材的首/中/末帧
      payload/metadata/U16 golden、损坏输入和容量 matrix；在该外部输入到位前不得宣称
      Batch E 全格式验收完成；GPU pipeline 边界保持冻结在 U16 RAW upload；
- [ ] 性能开发期间持续推进 NLE/硬件/长时间发布 gates。

Stage 0～3 / Batch A～D、Batch D.1-A～D 以及 Batch E-A～E-C 已完成；D.1-C/D 的
queue/staging 候选与 E-D compression 7 fast decoder 均记录 no-go。Batch E 当前唯一未关闭的
硬门槛是 **E-F 真实 compression 6 corpus**。在取得该素材前，不以 synthetic 或
compression 7 结果替代 legacy bitstream 的 bit-exact、安全和容量验收。
