...
- 可选的线性 DWG Capture Sharpening，以及使用逐帧 DNG `NoiseProfile`
  噪声模型的 Bayer 色度空间降噪；两者默认关闭。
...
```json
{
  "schema_version": 1,
  "demosaic": "rcd",
  "capture_sharpening": 0.25,
  "capture_sharpening_threshold": 0.002,
  "raw_chroma_denoise": 0.0
}
```

`demosaic` 可设为 `rcd`、`amaze`、`igv`、`dcb` 或 `lmmse`。Capture
Sharpening 在相机 RGB 转换到线性 DWG 后、DaVinci Intermediate OETF 前增强中性亮度细节；
阈值同样在线性 DWG 域。`raw_chroma_denoise` 在去马赛克前只修改 Bayer 红/蓝位置的
色差残差，并保持绿色样本不变。它使用 MCRAW 每帧 `noiseProfile` 提供的 DNG
`S/O` 噪声模型；启用时若元数据缺失会明确报错，不会猜测噪声参数。

可直接使用仓库中的 `config/compare-amaze.json`、`config/compare-dcb.json`、
`config/compare-lmmse.json`、`config/compare-capture-sharpening.json` 和
`config/compare-raw-chroma-denoise.json`。
...
- 当前 RAW 降噪仅为可选的 NoiseProfile 驱动色度空间滤波；时域降噪、亮度降噪、
  Vulkan、坏点、镜头阴影、几何校正、其他 Log profile 和 GUI 不在 v0.1 范围。
